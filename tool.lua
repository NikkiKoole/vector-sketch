...
function mylib:load(arg)
    ...
    profiling = false
    simplifyValue = 0.1
    getRidOfCounterClockwiseSVgContours = false
    scrollviewOffset = 0
...
local function getDataFromFile(file)
    ...
    if text.ends_with(filename, '.svg') then
        local command = 'node ' ..
            'resources/svg_to_love/index.js ' ..
            filename .. ' ' .. simplifyValue .. ' ' .. (getRidOfCounterClockwiseSVgContours and 1 or 0)
        print(command)
    ...
    if openFileScreen then
        ...
        if true then
                                ...
                                if (currentNode.children[i].mesh) then
                                    ...
                                    if currentNode.children[i].perspectiveMesh then
                                        if #result ~= currentNode.children[i].perspectiveMesh:getVertexCount() then
                                            currentNode.children[i].perspectiveMesh = LG.newMesh(formats.simple_format,
                                                result,
                                                "triangles", "stream")
                                        else
            ...
            if (editingMode == 'rectangle-select' or editingModeSub == 'rectangle-point-select') and rectangleSelect.startP
                ...
                LG.line(rectangleSelect.startP.x, rectangleSelect.startP.y, rectangleSelect.endP.x,
                    rectangleSelect.startP.y)
                LG.line(rectangleSelect.startP.x, rectangleSelect.endP.y, rectangleSelect.endP.x, rectangleSelect.endP.y)
                LG.line(rectangleSelect.startP.x, rectangleSelect.startP.y, rectangleSelect.startP.x,
                    rectangleSelect.endP.y)
            ...
            if (editingMode == 'folder' and currentNode and currentNode.transforms) then
                if (showTheParentTransforms) then
                    ...
                    if (currentNode.transforms.l[4] == currentNode.transforms.l[5]) then
                        local v = h_slider("folder-scale-xy", calcX(1), calcY(2), scrollerWidth,
                            currentNode.transforms.l[5],
                            0.00001, 10)
                        if (v.value ~= nil) then
                    ...
                    local v = h_slider("folder-scale-x", calcX(1), calcY(3), scrollerWidth, currentNode.transforms.l[4],
                        -2, 2)
                    if (v.value ~= nil) then
                    ...
                    local v = h_slider("folder-scale-y", calcX(1), calcY(4), scrollerWidth, currentNode.transforms.l[5],
                        -2, 2)
                    if (v.value ~= nil) then
                    ...
                    local v = h_slider('folder_skew_x', calcX(1), calcY(5), scrollerWidth,
                        currentNode.transforms.l[8] or 0,
                        -math.pi, math.pi)
                    if (v.value ~= nil) then
                    ...
                    local v = h_slider('folder_skew_y', calcX(1), calcY(6), scrollerWidth,
                        currentNode.transforms.l[9] or 0,
                        -math.pi, math.pi)
                    if (v.value ~= nil) then
            ...
            if (editingModeSub == 'polyline-palette' and currentNode and currentNode.color) then
                ...
                local thumbSize = 14
                local colorsInRow = 10 --math.floor(w / (thumbSize))
                ...
                for i = 1, #palette.colors do
                    ...
                    local x = xOffset + ((i - 1) % colorsInRow) * (thumbSize)
                ...
                LG.setColor(1, 1, 1, 1)
                LG.print("alpha", labelPos(calcX(0), calcY(5)))
                local v = h_slider("polyline_alpha", calcX(0), calcY(5), 100, currentNode.color[4], 0, 1)
                if (v.value ~= nil) then
                    currentNode.color[4] = v.value
                    LG.print(tostring(currentNode.color[4]), calcX(0), calcY(5))
                end
            end
            ...
            if editingMode ~= 'dopesheet' then
                ...
                local totalHeightGraphNodes = renderGraphNodes(root, 0, sceneGraph.topY, sceneGraph.x,
                    sceneGraph.height - 20)
                LG.setScissor()
            ...
            if fileDropPopup then
                ...
                if text.ends_with(name, 'polygons.txt') or text.ends_with(name, '.svg') then
                    if text.ends_with(name, '.svg') then
                        local vv = h_slider('simplify', 100, 100, 200, simplifyValue, 0, 5)
                        LG.setColor(0, 0, 0)
                        LG.print('simplfy', 100, 100)
                        if vv.value then
                            simplifyValue = vv.value
                        end

                        if getRidOfCounterClockwiseSVgContours then
                            love.graphics.setColor(0.1, 0.1, 0.1)
                        else
                            love.graphics.setColor(1, 0, 1)
                        end
                        love.graphics.circle('fill', 100, 150, 20)
                        LG.setColor(0, 0, 0)
                        LG.print(getRidOfCounterClockwiseSVgContours and 'no holes' or 'holes', 100, 150)
                        local ccwToggle, hover = ui.getUICircle(100, 150, 20)
                        if ccwToggle then
                            getRidOfCounterClockwiseSVgContours = not getRidOfCounterClockwiseSVgContours
                        end
...
function mylib:keypressed(key, scancode, isrepeat)
    ...
    if not console.isEnabled() then
        ...
        if key == 'a' and not changeName then
            ...
            render.renderNodeIntoCanvas(root, LG.newCanvas(1024 * 4, 1024 * 4),
                shapePath .. shapeName .. ".x4.polygons.png")
        end
