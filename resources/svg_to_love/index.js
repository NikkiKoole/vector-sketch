...
function calculateWindingOrder(points) {
  let windingNumber = 0;
  const numPoints = points.length;

  for (let i = 0; i < numPoints; i++) {
    const currentPoint = points[i];
    const nextPoint = points[(i + 1) % numPoints]; // Wrap around to the first point
  ...
  // Determine the winding order based on the sign of the winding number
  if (windingNumber > 0) {
    return "clockwise"; // Positive winding number indicates clockwise order
  } else if (windingNumber < 0) {
    return "counterclockwise"; // Negative winding number indicates counterclockwise order
  } else {
    return "undefined"; // Winding number is zero, the order is undefined
  }
}
...
function normalizeContours(contours, width, height, newWidth, newHeight) {
  ...
  contours.forEach((c) => {
    ...
    c.forEach((point) => {
      newContour.push([
        (point[0] / width) * newWidth,
        (point[1] / height) * newHeight,
      ]);
    });
...
function makeLoveShapePositions(
  fill,
  opacity,
  contours,
  groupIndex,
