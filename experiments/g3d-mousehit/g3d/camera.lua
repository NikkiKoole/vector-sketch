...
function camera:getMouseLookTarget(mx,my)
   ...
   local t = newMatrix()
   local t2 = newMatrix()
   local b  = t:setTransformationMatrix(camera.position, {rchange, 0, pchange}, {1,1,1})
   local a = t2:setTransformationMatrix(camera.position, {fpsController.direction, 0, fpsController.pitch}, {1,1,1})
   --print(a)
   print(a:multiply(b))
   --print(b)
   --print(a)
   ...
   --print(fpsController.pitch + pchange, math.asin(a[5]))
   --print(fpsController.direction + rchange, math.atan( - a[7], a[6]))
   
   
   local tempR = fpsController.direction + rchange 
   local tempP = fpsController.pitch + pchange

   --local tempR =  math.atan( - a[7], a[6]) -- * 1.2 --fpsController.direction + rchange * 1.4
   --local tempP = math.asin(a[5])
