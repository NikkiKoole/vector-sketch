...
function love.load()
    ...
    for _, key in ipairs(keys) do
        syllables[key] = love.audio.newSource("syllables/" .. key .. ".wav", "static")
    end
...
function love.keypressed(key)
    local keymap = {
        ['a'] = function() say('{root=g2} MI-po? PI-MO ?ka-ka-ka ja-ja ki-ka ko-ko?') end,
        ['b'] = function() say('?mi-mi? MI-mi? MI-mi-mi-[+2]mi') end,
        ['c'] = function() say('?mi-mi?') end,
...
function playSyllable(s, curveName, emphasized, riseOffset, defaultPitch)
    ...
    local pitchCurve = love.math.newBezierCurve(pitchCurves[curveName or 'neutral'])
    local activeSource = {
        emphasized = emphasized,
        source = originalSource,
        defaultPitch = defaultPitch or 1,
        pitchCurve = pitchCurve,
        riseOffset = riseOffset or 0,
...
function randomSayableString()
    ...
    for _ = 1, 4 do
        local syllCount = love.math.random(1, 2)
        local word = {}
    ...
    for wordToken in text:gmatch("%S+") do
        if wordToken == "," then
            ...
            for i, sData in ipairs(syllablesInWord) do
                local riseOffsetVal = rise and (0.05 * (i - 1)) or 0
                ...
                table.insert(sayQueue, {
                    ...
                    pitchCurveName = currentPitchCurveName or (sData.emphasized and 'emphasis-first') or 'neutral',
