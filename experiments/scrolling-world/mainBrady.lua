...
closest = mobileCam:addLayer( 'closest', 1.5, { relativeScale = (1.0/1.5) * 0.90 } )
close = mobileCam:addLayer( 'close', 1.25, { relativeScale = (1.0/1.25) * 0.95 } )
far = mobileCam:addLayer( 'far', .85, { relativeScale = (1.0/.85) * 1.05 } )
fartest = mobileCam:addLayer( 'fartest', .75, { relativeScale = (1.0/.75) * 1.1 } )

local overviewCam = Camera(
   W / 2 - 2 * offset,
   H - 2 * offset,
   {
      x = W / 2 + offset, y = offset, resizable = true, maintainAspectRatio = true,
      resizingFunction = function( self, w, h )
         resizeCamera( self, w, h )
         local W, H = love.graphics.getDimensions()
         self.x = W / 2 + offset
...
function love.load()
   love.window.setMode(1024, 768, {resizable=true, vsync=true, minwidth=400, minheight=300})
end
...
function love.draw()
   ...
   fartest:push()
   love.graphics.setColor( 1, 0, 1, .25 )
   drawSquares()
   fartest:pop()
   far:push()
