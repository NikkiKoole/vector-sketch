...
local softsurface = setmetatable({}, {
    __call = function(self, world, x, y, r, s, t, options, texture)
        local new = copy(self);
...
function softsurface:init(world, points, precision, mode, options, texture)
    options          = options or {}
    self.stiffness   = options.stiffness or 1   -- Default stiffness
    self.damping     = options.damping or 0.5   -- Default damping
    ...
    for i = 1, #t, 4 do
        ...
        local base_body    = physics.newBody(world, x1 + dx * (distance_to / 2 + precision / 4),
            y1 + dy * (distance_to / 2 + precision / 4), mode)
        ...
        -- Apply friction and restitution to the base fixture
        base_fixture:setFriction(self.friction)
        base_fixture:setRestitution(self.restitution)
        ...
        table.insert(self.phys,
            { body = base_body, shape = base_shape, fixture = base_fixture, joint_a = joint_a, joint_b = joint_b })
        ...
        for j = 1, surface_count do
            ...
            joint_a:setDampingRatio(self.damping) -- Apply damping ratio
            joint_a:setFrequency(self.stiffness)  -- Apply stiffness (frequency)
            ...
            if j > 1 or (i > 1 and j == 1) then
                ...
                if i > 1 and j == 1 then
                    joint_b = physics.newRopeJoint(v.body, body, v.body:getX() + last_dx * (precision / 2),
                        v.body:getY() + last_dy * (precision / 2),
                        x - dx * (precision / 2), y - dy * (precision / 2), 1, false)
                else
                    joint_b = physics.newRopeJoint(v.body, body, v.body:getX() + dx * (precision / 2),
                        v.body:getY() + dy * (precision / 2),
            ...
            if i >= #t - 4 and j == surface_count then
                local v = self.phys[2]
                joint_c = physics.newRopeJoint(v.body, body, v.body:getX() - first_dx * (precision / 2),
                    v.body:getY() - first_dy * (precision / 2),
                    x + dx * (precision / 2), y + dy * (precision / 2), 5, false)
            end

            -- store it
            table.insert(self.phys,
                { body = body, shape = shape, fixture = fixture, joint_a = joint_a, joint_b = joint_b, joint_c = joint_c })
        end
...
function softsurface:setStiffness(stiffness)
    self.stiffness = stiffness
    for i, physObj in ipairs(self.phys) do
        -- Adjust the stiffness of the joints (if applicable)
        if physObj.joint_a then
            physObj.joint_a:setFrequency(self.stiffness)
        end
    end
end

function softsurface:setDamping(damping)
    self.damping = damping
    for i, physObj in ipairs(self.phys) do
        -- Adjust the damping ratio of the joints
        if physObj.joint_a then
            physObj.joint_a:setDampingRatio(self.damping)
        end
    end
end

function softsurface:setFriction(friction)
    self.friction = friction
    for i, physObj in ipairs(self.phys) do
        -- Adjust the friction of the fixture
        physObj.fixture:setFriction(self.friction)
    end
end

function softsurface:setRestitution(restitution)
    self.restitution = restitution
    for i, physObj in ipairs(self.phys) do
        -- Adjust the restitution (bounciness) of the fixture
        physObj.fixture:setRestitution(self.restitution)
    end
end
