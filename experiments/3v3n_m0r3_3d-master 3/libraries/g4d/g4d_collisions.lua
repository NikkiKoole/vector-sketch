...
function Collisions:generate_aabb(vertices, parentTransform)
	local aabb = {
...
function Collisions:collide_with_aabb(other)
...
function Collisions:collide_with_point(x, y, z)
...
function Collisions:get_distance_from(x, y, z)
	if type(x) == 'table' then x, y, z = x[1], x[2], x[3] end

	return math.sqrt((x - self.x)^2 + (y - self.y)^2 + (z - self.z)^2)
end

function Collisions:collide_with_directional_ray(src, dir) -- vec3 position, vec3 direction
	local src_x, src_y, src_z = src[1], src[2], src[3]
	local dir_x, dir_y, dir_z = Vectors:fast_normalize(dir[1], dir[2], dir[3])
...
function Collisions:collide_with_positional_ray(src, target) -- vec3 position, vec3 target
...
-- plane_position - vec3
-- plane_normal   - vec3
function Collisions:ray_plane(ray_position, ray_direction, plane_position, plane_normal)
	local denom = Vectors:dot_product(plane_normal, ray_direction)

	if math.abs(denom) < DBL_EPSILON then return false end -- ray does not intersect plane

	local direction                = Vectors:subtract(plane_position, ray_position) 
	local distance_from_ray_origin = Vectors:dot_product(direction, plane_normal) / denom

	if distance_from_ray_origin < DBL_EPSILON then return false end

	local intersection_point = Vectors:add(ray_position, Vectors:scalar_multiply(ray_direction, distance_from_ray_origin))
...
function Collisions:ray_triangle(ray_position, ray_direction, triangles, backface_cull)
	local e1 = Vectors:subtract(triangles[2], triangles[1])
	local e2 = Vectors:subtract(triangles[3], triangles[1])
	local h  = Vectors:cross_product(ray_direction, e2)
	local a  = Vectors:dot_product(h, e1)
	...
	local f = 1 / a
	local s = Vectors:subtract(ray_position, triangles[1])
	local u = Vectors:dot_product(s, h) * f

	if u < 0 or u > 1 then return false end

	local q = Vectors:cross_product(s, e1)
	local v = Vectors:dot_product(ray_direction, q) * f

	if v < 0 or u + v > 1 then return false end

	local distance_from_ray_origin = Vectors:dot_product(q, e2) * f
...
function Collisions:closest_point(src_x, src_y, src_z)
	return find_closest(self, self.verts, triangle_point, src_x, src_y, src_z)
end

function Collisions:ray_intersection(src_x, src_y, src_z, dir_x, dir_y, dir_z)
    return find_closest(self, self.verts, triangle_ray, src_x, src_y, src_z, dir_x, dir_y, dir_z)
end

function Collisions:sphere_intersection(src_x, src_y, src_z, radius)
    return find_closest(self, self.verts, triangle_sphere, src_x, src_y, src_z, radius)
end

function Collisions:capsule_intersection(tip_x, tip_y, tip_z, base_x, base_y, base_z, radius)
    -- the normal vector coming out the tip of the capsule
    local norm_x, norm_y, norm_z = Vectors:fast_normalize(tip_x - base_x, tip_y - base_y, tip_z - base_z)
...
function Collisions:create_collision_zone(zone_size)
    ...
    for x=min_1, max_1, zone_size do
        for y=min_2, max_2, zone_size do
            for z=min_3, max_3, zone_size do
                ...
                for v=1, #verts, 3 do
                    local n_x, n_y, n_z = Vectors:fast_normalize(
                        verts[v][6]*scale_x,
                        verts[v][7]*scale_x,
                        verts[v][8]*scale_x
                    )
...
local function closest_point_on_line_segment(a_x,a_y,a_z, b_x,b_y,b_z, x,y,z)
    ...
    local t = Vectors:fast_dot_product(x - a_x, y - a_y, z - a_z, ab_x, ab_y, ab_z) / (ab_x^2 + ab_y^2 + ab_z^2)
...
local function triangle_ray(
    ...
    local e11, e12, e13 = Vectors:fast_subtract(tri1_x,tri1_y,tri1_z, tri0_x,tri0_y,tri0_z)
    local e21, e22, e23 = Vectors:fast_subtract(tri2_x,tri2_y,tri2_z, tri0_x,tri0_y,tri0_z)
    local h1 , h2 , h3  = Vectors:fast_cross_product(dir_x,dir_y,dir_z, e21,e22,e23)
    local a             = Vectors:fast_dot_product(h1,h2,h3, e11,e12,e13)

    -- if a is too close to 0, ray does not intersect triangle
    if math.abs(a) <= DBL_EPSILON then return end

    local s1, s2, s3 = Vectors:fast_subtract(src_x,src_y,src_z, tri0_x,tri0_y,tri0_z)
    local u          = Vectors:fast_dot_product(s1,s2,s3, h1,h2,h3) / a

    -- ray does not intersect triangle
    if u < 0 or u > 1 then return end

    local q1, q2, q3 = Vectors:fast_cross_product(s1,s2,s3, e11,e12,e13)
    local v          = Vectors:fast_dot_product(dir_x,dir_y,dir_z, q1,q2,q3) / a
    ...
    local thisLength = Vectors:fast_dot_product(q1,q2,q3, e21,e22,e23) / a
    ...
    if thisLength >= DBL_EPSILON and (not finalLength or thisLength < finalLength) then
        --local norm_x, norm_y, norm_z = Vectors:fast_cross_product(e11,e12,e13, e21,e22,e23)
...
local function triangle_sphere(tri0_x, tri0_y, tri0_z, tri1_x, tri1_y, tri1_z, tri2_x, tri2_y, tri2_z, trin_x, trin_y, trin_z, src_x, src_y, src_z, radius)
    ...
    local n_x, n_y, n_z = Vectors:fast_normalize(Vectors:fast_cross_product(side1_x, side1_y, side1_z, side2_x, side2_y, side2_z))

    -- distance from src to a vertex on the triangle
    local dist = Vectors:fast_dot_product(src_x - tri0_x, src_y - tri0_y, src_z - tri0_z, n_x, n_y, n_z)
    ...
    local c0_x, c0_y, c0_z = Vectors:fast_cross_product(itx_x - tri0_x, itx_y - tri0_y, itx_z - tri0_z, tri1_x - tri0_x, tri1_y - tri0_y, tri1_z - tri0_z)
    local c1_x, c1_y, c1_z = Vectors:fast_cross_product(itx_x - tri1_x, itx_y - tri1_y, itx_z - tri1_z, tri2_x - tri1_x, tri2_y - tri1_y, tri2_z - tri1_z)
    local c2_x, c2_y, c2_z = Vectors:fast_cross_product(itx_x - tri2_x, itx_y - tri2_y, itx_z - tri2_z, tri0_x - tri2_x, tri0_y - tri2_y, tri0_z - tri2_z)
    if  Vectors:fast_dot_product(c0_x, c0_y, c0_z, n_x, n_y, n_z) <= 0
    and Vectors:fast_dot_product(c1_x, c1_y, c1_z, n_x, n_y, n_z) <= 0
    and Vectors:fast_dot_product(c2_x, c2_y, c2_z, n_x, n_y, n_z) <= 0 then
        ...
        if n_x == 0 and n_y == 0 and n_z == 0 then
            return Vectors:fast_magnitude(n_x, n_y, n_z), itx_x, itx_y, itx_z, trin_x, trin_y, trin_z
        end

        return Vectors:fast_magnitude(n_x, n_y, n_z), itx_x, itx_y, itx_z, n_x, n_y, n_z
    end
...
local function triangle_capsule(
    ...
    local dotOfNormals = math.abs(Vectors:fast_dot_product(n_x, n_y, n_z, capn_x, capn_y, capn_z))
    ...
    if dotOfNormals > 0 then
        ...
        local t = Vectors:fast_dot_product(n_x, n_y, n_z, (tri0_x - base_x) / dotOfNormals, (tri0_y - base_y) / dotOfNormals, (tri0_z - base_z) / dotOfNormals)
