...
local Shaders    = require(G4D_PATH .. '/g4d_shader_loader')('all')
...
function Camera:resize(w,h)
   ...
   self:update_projection_matrix('projection')
...
function Camera:update_view_matrix(dt)
	for k, shader in pairs(Shaders) do 
		...
		if shader:hasUniform('camera.position') then 
			shader:send('camera.position', {self.x, self.y, self.z, 1})
		end
	end
end
...
function Camera:update_projection_matrix(type)
	...
	for k, shader in pairs(Shaders) do 
		shader:send('camera.projection_matrix', matrix)
	end
end

function Camera:get_projection_matrix()
	return Matrices:get_projection_matrix(self.fov, self.near_clip, self.far_clip, self.aspect_ratio)
end

function Camera:get_view_matrix()
	return Matrices:get_view_matrix({self.x, self.y, self.z}, {self.tx, self.ty, self.tz}, self.down)
end

function Camera:get_inverse_projection_matrix()
	return Matrices:invert(self:get_projection_matrix())
end

function Camera:get_inverse_view_matrix()
	return Matrices:invert(self:get_view_matrix())
end

function Camera:is_world_point_on_screen(point) -- vec3
	local width, height = love.graphics.getDimensions()
	local pv_matrix     = Matrices:multiply(self:get_projection_matrix(), self:get_view_matrix())

	local pos = Matrices:multiply_vector(pv_matrix, {point[1], point[2], point[3], 1})
	pos[1] = pos[1] / pos[4]
	pos[2] = pos[2] / pos[4]
	pos[3] = pos[3] / pos[4]
	pos[4] = pos[4] / pos[4]
	...
	local cam_dir     = self:get_center_ray()
	local targ_to_cam = self:get_target_ray(point)
	local dot_product = Vectors:dot_product(cam_dir, targ_to_cam)
...
function Camera:get_center_ray()
	local width, height = love.graphics.getDimensions()
...
function Camera:get_target_ray(target)
	return Vectors:normalize(Vectors:subtract(target, self:position()))
end
...
function Camera:get_screen_ray(x, y)
	...
	local eye_coord   = Matrices:multiply_vector(self:get_inverse_projection_matrix(), clip_coord)
	local world_coord = Matrices:multiply_vector(self:get_inverse_view_matrix(), {eye_coord[1], eye_coord[2], -1, 0})
