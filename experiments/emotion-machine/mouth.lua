...
local backdrop = love.graphics.newImage('assets/type4.png')
backdrop:setWrap('repeat', 'repeat')
local teeth = love.graphics.newImage('assets/teeth4.png')
...
function Mouth:drawShape(shape, scaleX, scaleY)
    ...
    love.graphics.setColor(0, 0, 1, .4)
    ...
    if true then
        ...
        texturedCurve(upCurve, self.lip, m1, -1, 0.3, self.lipoffset)
        love.graphics.draw(m1)

        local m2 = createTexturedTriangleStrip(self.lip)
        texturedCurve(downCurve, self.lip, m2, -1, 0.3, -self.lipoffset)
