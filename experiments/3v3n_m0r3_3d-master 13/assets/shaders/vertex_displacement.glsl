...
vec4 position(mat4 transform_projection, vec4 initial_vertex_position)
{
	...
	float motion1 = noise(vec2(vertex_position.x    + model.time, vertex_position.z    + model.time));
	float motion2 = noise(vec2(vertex_position.x+1. + model.time, vertex_position.z+1. - model.time));
	float motion3 = noise(vec2(vertex_position.x+2. - model.time, vertex_position.z+2. - model.time));
	float motion4 = noise(vec2(vertex_position.x+3. - model.time, vertex_position.z+3. + model.time));

	float mix1 = mix(motion1, motion2, .5);
	float mix2 = mix(motion3, motion4, .5);
	final_mix  = mix(mix1, mix2, .5) - .5;

	initial_vertex_position.y += (final_mix * 2);
...
{
	return vec4(0, .5 + final_mix, .5 + final_mix, 1);
}
#endif
