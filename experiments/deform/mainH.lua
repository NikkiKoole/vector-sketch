-- main.lua
-- Box2D → Bones → Organic Skinned Mesh + Mouse Interaction
-- Three bodies: TORSO (static) + UPPER (shoulder) + LOWER (elbow) with revolute joints.
-- Physics drives 3 bones. An organic ribbon mesh is skinned across torso→upper→lower.
-- NEW: Use a MouseJoint to grab and drag dynamic bodies (upper/lower) with the mouse.
-- LÖVE 11.x
...
function love.load()
    love.window.setTitle("Box2D → Bones → Organic Skinned Arm + Mouse Drag")
    ...
    local function cb(fixture)
        if fixture:testPoint(x, y) then
            ...
            if b:getType() == "dynamic" then
                found = b; return false -- stop searching
            end
        end
        return true
    end
...
function love.mousepressed(x, y, button)
    if button == 1 then
        mouseX, mouseY = x - 80, y - 60 -- account for our drawing translate
