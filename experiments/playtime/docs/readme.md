...
- **`MIPOLAI-COMMON.md`** — the cross-app template: studio-shared engine
  primitives (camera, scene manager + screen transitions, mipomi-lang speech,
  emotion layer, physical actions, tuning panel) and studio-shared mechanics
  (breeds, shelf+paywall, payoff beat, footage capture). Catalog with file
  pointers + gap analysis per item.
