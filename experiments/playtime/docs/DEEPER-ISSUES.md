...
**~~Issue~~**: ~~The curve control point computation uses body angles directly. If bodies rotate wildly during physics simulation, the curve can flip inside-out or create self-intersections. There's no clamping or smoothing.~~ — **FIXED (2026-04-13)**: `texturedCurve` now has per-sample miter-limit width clamping: `halfWidth ≤ min(prevSeg, nextSeg) × tan((π − turnAngle) / 2)`. The ribbon's inner edge can no longer cross itself at sharp bends (bow-tie artifact at flexed elbows/knees). Straight joins are unaffected.
...
## 13. UV/Backdrop/Texture Fragilities — FIXED (partially)

See `docs/UV-BACKDROP-FRAGILITY.md` for the full briefing. Key fixes applied 2026-04-13:

### ~~Bug: UV lookup loop matched x and y coords independently~~ — FIXED

The MESHUSERT and UVUSERT render paths (`box2d-draw-textured.lua`) had a UV lookup loop that compared each triangle vertex's x against ALL slots in the interleaved `{x1,y1,x2,y2,...}` array, and y similarly. If a polygon had `x-coord == y-coord` at different vertices, UVs got scrambled. Replaced with pair-keyed lookup (`buildPolyVertexIndex` + `lookupUV`) that maps by full `(x,y)` tuple.

### ~~Bug: Stale UV array after polygon vertex edit~~ — FIXED

RESOURCE sfixture's `uvs` array was sized by vertex count at compute time but never invalidated when the polygon was edited (add/remove vertex) or cloned. Added a dirty hook in `object-manager.lua:recreateThingFromBody`: when `#oldVertices ~= #newVertices`, any RESOURCE sfixture on the body has its `ud.extra.uvs` nulled.

### ~~Bug: MESHUSERT deformation candy-wrapper collapse~~ — FIXED

`deformWorldVerts` used Linear Blend Skinning (LBS) which collapses at joints when influencing bodies rotate oppositely. Added Dual Quaternion Skinning (DQS) as default (`lib.useDQS = true`). DQS blends delta rotations via circular mean, preserving mesh volume. Requires `bindAngle` per influence + `bindVerts` per vertex; captured at bind time, backfilled on load.

### Remaining fragilities (see UV-BACKDROP-FRAGILITY.md)

- Backdrop index coupling to global array (URL fallback mitigates across sessions)
- UV normalization pins to backdrop position at compute time (no invalidation on backdrop move)
- Label-based mapper lookup has no uniqueness guarantee
- Backdrop images lazy-loaded in draw loop (crash at render time, not load time)
...
| UV/texture fragilities | 7 | 3 | 4 | See docs/UV-BACKDROP-FRAGILITY.md for full list |
