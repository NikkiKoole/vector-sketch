...
function softbody:init(world, x, y, r, s, t, reinit)
    ...
    self.centerShape = physics.newCircleShape(r / 2);
    ...
    for i = 1, #self.nodes do
        if i < #self.nodes then
            local j = physics.newDistanceJoint(self.nodes[i].body, self.nodes[i + 1].body, self.nodes[i].body:getX(),
                self.nodes[i].body:getY(),
                self.nodes[i + 1].body:getX(), self.nodes[i + 1].body:getY(), false);
            self.nodes[i].joint2 = j;
        else
            local j = physics.newDistanceJoint(self.nodes[i].body, self.nodes[1].body, self.nodes[i].body:getX(),
                self.nodes[i].body:getY(),
