...
### Bug: Operator precedence in cloneSelection (io.lua:884)

```lua
if not doneJoints[ud.id] == true then
```

Due to Lua operator precedence, `not` binds tighter than `==`. This evaluates as:

```lua
if (not doneJoints[ud.id]) == true then
```

Which happens to work when `doneJoints[ud.id]` is `nil` (falsy → `not nil` = `true` = `true`), but **fails when the value is already `true`**: `not true` = `false`, `false == true` = `false`. So the guard does work — but only by accident. The intended code was likely:

```lua
if not (doneJoints[ud.id] == true) then
-- or simply:
if not doneJoints[ud.id] then
```

This one works by luck, but it's the kind of bug that a future edit could break.
...
### Bug: Debug print left in production code (joints.lua:104)

```lua
print('apjspaiosjdposdjf')
```

This fires when someone tries to create a joint between a body and itself. Instead of a proper error message, it prints gibberish and silently returns.

### Bug: `if true then` blocks (8 occurrences)

These appear in joints.lua (lines 266, 296) and box2d-draw-textured.lua (lines 318, 358, 495). They're leftover from debugging — the developer wrapped code in `if true then` to toggle it on/off, then forgot to remove the wrapper. They don't cause errors but signal unfinished cleanup.
...
## Summary: New Bug/Issue Count

| Category | Count | Examples |
|----------|-------|---------|
| Actual bugs | 5 | Operator precedence in clone, OMP dirty flag, debug print, unused swapBodies, redundant reference angle read |
| Hidden constraints | 2 | Fixture ordering invariant, z-sort instability |
| Global leaks (new) | 9 | playtime-ui.lua functions defined without local |
| State mutation risks | 4 | currentMode (27 writers), paused (3 writers), selection (scattered), direct setUserData bypass |
| Memory leaks | 3 | Image cache never cleared, OMP images not released, canvas allocation |
| Missing features | 2 | LIMB_HUB behavior never implemented, behaviors system is a stub |
| Silent failure paths | 3+ | Clone influence nil, clone joint skip, fixture ordering |
| Dead/debug code | 8+ | `if true then` blocks, gibberish print, disabled collision checks |

**Updated global leak total: ~40** (was ~30 in MODULE-ANALYSIS.md).
