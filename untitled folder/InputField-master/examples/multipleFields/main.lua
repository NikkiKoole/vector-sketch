...
local ENABLE_CJK              = true
local COMPOSITION_BOX_PADDING = 3

local theFont = ENABLE_CJK and LG.newFont("Windsor_BT.ttf", 24) or LG.newFont(16)
...
for _, textInput in ipairs(textInputs) do
   ...
   if (textInput.kind and textInput.kind == 'numeric') then
      textInput.field:setFilter(function(a)
	    local filter =  a:match("%d")
	    if filter == nil then
	       if a == ',' or a == '.' or a == '-' then
		  filter = a
	       end
