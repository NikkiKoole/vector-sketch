...
function handleChild(shape, isDirty)
   ...
   if currentNode ~= shape then
      ...
      if shape.texture and shape.texture.canvas then
         ...
         -- todo dont regenerate all the time!!
         local texture = shape.texture.canvas
         local data = texture:newImageData()
         local img = love.graphics.newImage(data)

         local editing = mesh.makeVertices(shape)

         mesh.addUVToVerts(editing, img, shape.points, shape.texture)
         local made = mesh.makeMeshFromVertices(editing, shape.type, shape.texture)
